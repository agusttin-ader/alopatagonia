    ...
    >
      ...
      <div className="relative z-[2] mx-auto flex min-h-[min(86vh,880px)] w-full max-w-3xl flex-col items-center justify-center px-4 py-24 text-center sm:px-8 sm:py-28 lg:max-w-4xl lg:px-12 lg:py-32">
        ...
        <div className="mt-8 max-w-2xl space-y-5 text-base leading-relaxed text-white/92 sm:mt-9 sm:space-y-6 sm:text-lg lg:text-[1.125rem] lg:leading-[1.65] [text-shadow:0_1px_6px_rgba(0,0,0,0.38)]">
          {ABOUT_US_COPY.paragraphs.map((paragraph) => (
            <p key={paragraph}>{paragraph}</p>
          ))}
          <p className="font-semibold text-white">{ABOUT_US_COPY.closing}</p>
        </div>
        ...
        <a
          ...
          className={cn(
            ...
            "motion-cta mt-10 inline-flex h-12 rounded-full px-10 text-base font-semibold sm:mt-11",
          )}
        >
          Contactanos
        </a>
